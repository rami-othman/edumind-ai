...
اذكر المصادر المتاحة في نهاية الإجابة بهذا الشكل عند توفر البيانات:
المصادر:
- الملف: ohms_law_arabic.pdf، الصفحة: 4، المقطع: 3
يمكنك إضافة المادة والدرس فقط إذا كانت موجودة في بيانات المصدر.
لا تخترع أرقام صفحات أو مصادر أو بيانات غير موجودة في السياق."""
...
- اشرح خطوة بخطوة عندما يكون ذلك مفيدا.
- استخدم أمثلة بسيطة فقط إذا كانت مدعومة بالسياق.
- اذكر المصادر في النهاية بهذا الشكل عند توفر البيانات:
  المصادر:
  - الملف: ohms_law_arabic.pdf، الصفحة: 4، المقطع: 3
- إذا توفرت المادة أو الدرس في بيانات المصدر، يمكن ذكرهما في نفس سطر المصدر.
- إذا كان رقم الصفحة غير موجود، اذكر الحقول المتاحة فقط.
...
def _build_source(chunk: RetrievedChunk) -> dict[str, object]:
    ...
    for key in SOURCE_METADATA_FIELDS:
        ...
        if key in chunk.metadata and _is_simple_source_value(value):
            source[key] = value
